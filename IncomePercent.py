...
#define files for educational attainment/income data
...
#read in pandas dataframes for each year
for key, value in files.items():
    fileData = pd.read_csv(value, skiprows = 2, na_values=(['**','-']), names = (['County', 'Median Total', 
                    'MOE Median Total', 'Less Than High School','MOE Less Than High School', 
                    'High School', 'MOE High School', 'Some College/Associate', 
    ...
    #makes new column showing high school income as a percentage of Bachelor degree income for each county
    tempwest['Percentage']= tempwest['High School'] / tempwest['Bachelor']
    tempeast['Percentage']= tempeast['High School'] / tempeast['Bachelor']
    tempcentral['Percentage']= tempcentral['High School'] / tempcentral['Bachelor']
...
#finds the mean percentage for each region for each year
years = ['2010', '2011', '2012', '2013', '2014', '2016', '2017']
westmeans=[]
for year in years:
    x = west[year]['Percentage'].mean()
    westmeans.append(x)
eastmeans=[]
for year in years:
    x = east[year]['Percentage'].mean()
    eastmeans.append(x)
centralmeans=[]
for year in years:
    x = central[year]['Percentage'].mean()
    centralmeans.append(x)

#plot shows chnage in percentage of income over time
...
plt.title('High School Income as a Percentage of Bachelor Degree Income')
